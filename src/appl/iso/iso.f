...
*     FEBRUARY, 1997
*
*     Dynamic version based on the program by C. Froese Fischer, L. 
*     Smentek-Mielczarek, N. Vaeck, and G. Miecznik, 
*     Computer Phys. Commun.  74 (1993) 415
*
...
*     -----------------------------------------------------------------
*
*     READ THE WAVEFUNCTIONS FROM THE   <NAME>.W   FILE.
*
      SUBROUTINE ISOWF
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
      ...
      DIMENSION P(NOD)
      COMMON/ELLABEL/ELLAB(NWD)
      COMMON/PAPIL/NCI,NWI
      COMMON/INOUT/IN,IERR,IPRI,IUF
      COMMON/PRA/Z
      POINTER(IQAZ,AZ(1))
      COMMON/NEL1/IQAZ
      CALL ALLOC(IQAZ,NWI,8)
      I=1
    1 READ(IUF,END=2) ATOM,TERM,EL1,M,ZZ,ETI,EKI,AZ(I),(P(J),J=1,M)
      IF(EL1.NE.ELLAB(I)) THEN
	WRITE(IERR,*) 'Radial functions do not match electron list'
	WRITE(IERR,*) 'Read: ',EL1, ' should be ', ELLAB(I)
	PRINT *, ' i=',I,NWI
	STOP
      ENDIF
      I=I+1
