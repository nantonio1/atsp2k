...
9     FORMAT(/// 24X,5HATOM ,A6,3X,5HTERM ,A6//3X,13HMEAN VALUE OF,21X,
     1   22HONE ELECTRON INTEGRALS /5X,2HNL,7X,5HE(NL),9X,'I(nl)',
     2   5X,'KE(nl)', 5X, 'RelS', 5X, 'S(nl)', 5X, 'Az(nl)')
...
26    FORMAT(//5X,'ENERGY (a.u.)'/5X,'------'/
     ...
     : 10X,' Ratio              ',F16.9)
        ...
        if (pcoeff.ne.0) call dalloc(pcoeff,ncodim)
        if (pinptr.ne.0) call dalloc(pinptr,ncodim)
