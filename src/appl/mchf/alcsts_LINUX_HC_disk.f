...
!     qhmx = malloc(nze_max_bl*8)
!     pih = malloc(nze_tot*4)
      qhmx = 0  ! DBG code   ! this version uses only disk
                              ! comment the expression to make it dynamic
...
!        pico = malloc(nze_tot*4)
      ...
      if(ico_memory) then
        ...
        pcoeff = 0  ! Code to use disk for c.lst
        !pcoeff = malloc(new_size*8)
        !pinptr = malloc(new_size*4)
