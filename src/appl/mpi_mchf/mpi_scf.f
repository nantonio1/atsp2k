*    Modified for a single configuration in some block.  NOT TESTED!
*
*     ------------------------------------------------------------------
*    3-32      S C F
*     -----------------------------------------------------------------
...
*
* the following is needed for gdsum in update/updatc
...
*
      ...
      ECONV = .FALSE.
      LAST = .FALSE.
      !call MPI_BCAST(last,1,MPI_LOGICAL,0, MPI_COMM_WORLD,ierr)
      ...
      CALL DIAG(ECONV,ACFG,CFGTOL,LAST,icycle,eigst_weight,tmp,cf_tot)
...
12    CALL DIAG(ECONV,ACFG,CFGTOL,LAST,icycle,eigst_weight,tmp,cf_tot)
*
