...
*
        PARAMETER (NWD=60,NOD=220,NOFFD=800, MTERM=20,MEIG=20)
*
        INCLUDE 'mpif.h'
...
*
       ...
       !  print*, 'rr',IN,OUT,PRI,IUC,IUF,IUD,OUC,OUF,OUD,OUH,ISCW
      DO 1 I=1,NO
      R(I)= EXP(RHO)/Z
      RR(I) = R(I)*R(I)
      R2(I) = DSQRT(R(I))
1     RHO = RHO + H
!      if (myid == 1) print*, R(1:NOD),'x', RR(1:NOD),'xx', R2(1:NOD)
      RHO = RHO - NO*H
*
...
*           .. orthogonalize larger n to smaller
            ...
            IF ( DABS(PN) .GT. 1.D-8 ) THEN
              PNN = DSQRT(D1 - PN*PN) ! if(myid==1)print*, d1 ,pn, 2222
...
*     IF (E(I,I) .EQ. D0) E(I,I) = HL(EL,I,I,REL) - EKIN(I,I)
      ...
      IF ( IND(I) .EQ. -2 ) THEN
        ...
        ELSE
           TITLE = ' UNCHANGED'
      END IF
!      if (myid==1) print*, 'WAVFVEEVEV',P(1:nod)
