...
C
        ...
        PARAMETER (NWD=60,NOD=220,NOFFD=1600)
*
*     MPI stuff ***********************************************
*
	INCLUDE 'mpif.h'
...
1       YR(J) = D0
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
* The following distribution does not correspond to the matrix one.
* It simply facilitates a parallel execution of the loop
...
* * * * Same distribution here...
        DO 10 J = IBEGIN+myid,IEND,nprocs
           IE = 0
...
*             print*, 'SUMI = ', SUMI
 ...
 10     CONTINUE
* * * * * * * * * * 
* Global Summation of the partial results
* This is done in XCH at the same point with the other gdsum.
* POTL should be called just before XCH in grange.f and solve.f
*       call gdsum(yr,no,yk)
*
        END
