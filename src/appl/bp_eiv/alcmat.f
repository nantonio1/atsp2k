...
!CFF  This should be computed also for LS
!     if (iLS .eq. 0) call lsp_comp(ncfg,lsp,jj)
      call lsp_comp(ncfg,lsp,jj)
...
!       iqhmx = 0; ! DEBUG code
     ...
     :    n_hmx, '...dvdson ON DISK'
      ...
      if (.not.lhmx_memory) then
         idisk = 1;
         call alloc(iqhmx, nze_c, 8) 
         call alloc(iqimx, nze_c, 4)
