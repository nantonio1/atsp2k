...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
					write (6, advance = "YES", FMT="(f10.4,a1,a2)"), &
						100 * ptr % coef, "-", ptr % term_label
				else
					write (6, advance = "NO", &
						FMT="(f10.4,a1,a2,x1a1)"), 100 * ptr % coef, &
						"-", ptr % term_label, " "
				end if
				ptr => ptr % next
...
contains
	...
	subroutine display_list (head, cfgl, maxcfgl)
		...
		if (associated (head)) then
			...
			do
				if (cfgl) then
					write (6, advance="NO", fmt = '(a5,i4,a2)'), &
						ptr % LS_label, ptr % twoJ, "  "
					write (6, advance="NO", FMT='(a)') ptr % cnf &
						(1:maxcfgl -2)
					write (6, advance = "NO", FMT='(f16.8,a3)') &
						ptr % head % energy, '   '
				else
					write (6, advance="NO", fmt = '(a5,i4)'), &
						ptr % LS_label, ptr % twoJ
					write (6, advance = "NO", FMT='(f16.8,a3)') &
						ptr % head % energy, '   '
				end if
...
write (0, FMT='(a21)') "Enter name of .c file"
read  *, filename
write (0, FMT='(a33)') "Print configuration labels? (y/n)"
read  *, cfgans
...
if (ios1 /=0) then
	write (0, FMT='(a39)') "Input file with .c extension not found."
	stop
end if

if (ios2 /=0) then
	write (0, FMT='(a39)') "Input file with .j extension not found."
	stop
end if
...
do
	...
	do Jcntr = 0, Jnum - 1
		...
		config   = line1 (3:cnf + 1)
                !print *, 'LS_label, config: ', LS_label, '  ', config(1:20)
		...
		do j = 0, ((ncfg + 7) / 7) !was + 1
			...
			if (j < (ncfg  + 7) / 7) then !was + 1 with < 
				...
				do k = 0, 6
					...
					ind = index (line1 (:80), "    ")
                                        if (line1 (ind-3:ind-3) == ' ') then
					  LSterm = line1 (ind - 2:ind - 1)
                                        else
					  LSterm = line1 (ind - 3:ind - 2)
                                        end if  
                                        !Print *, 'LSterm:  ', LSterm
		...
		if (Jcntr == 0 .and. begin == 0) then
			...
			begin = 1
			if (cfgl == 0) then
				write (6,*) "Term  2*J      Energy      ", &
					"Leading percentages"
			else
				write (6, advance = "NO", fmt = '(a25)') &
					"Term  2*J   Configuration"
				do m = 0, maxcfgl - 11
					write (6, advance = "NO", FMT='(a1)') " "
				end do
				write (6,*) "Energy      Leading percentages"
			end if			
		end if
